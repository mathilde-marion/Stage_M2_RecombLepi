...
parser.add_argument("--recomb", required=True)  # fichier quantiles.tsv
...
genes = load_or_compute(tmpfile("tmp_01_genes_quantile.tsv"), compute_genes_raw)

# ────────────────────────────────────────────────
# STEP 2 : recomb (QUANTILES)
# ────────────────────────────────────────────────
def compute_recomb():
...
recomb = load_or_compute(tmpfile("tmp_02_recomb_quantile.tsv"), compute_recomb)

# ────────────────────────────────────────────────
# STEP 3 : Suppression duplicats
# ────────────────────────────────────────────────
def compute_genes_no_duplicates():
    return genes.drop_duplicates(subset=["chrom", "start", "end", "strand"])

genes = load_or_compute(tmpfile("tmp_03_genes_no_duplicates_quantile.tsv"), compute_genes_no_duplicates)
...
genes = load_or_compute(tmpfile("tmp_04_genes_final_quantile.tsv"), compute_genes_no_overlaps)

# ────────────────────────────────────────────────
# STEP 5 : fenêtres + classification
# ────────────────────────────────────────────────
...
recomb_windows = load_or_compute(
    tmpfile("tmp_05_recomb_windows_classified_quantile.tsv"),
    compute_recomb_windows
)

# ────────────────────────────────────────────────
# STEP 6 : ASSOCIER QUANTILE (clé)
# ────────────────────────────────────────────────
def compute_recomb_rate():
    ...
    for chrom in recomb_windows["chrom"].unique():
        ...
        def get_rate(pos):
            ...
            if len(inside) > 0:
                return inside.iloc[0]["recomb_quantile_100"]
            ...
            return (wb * b["recomb_quantile_100"] + wa * a["recomb_quantile_100"]) / (wb + wa)

        w["recombRateRelative"] = w["pos"].apply(get_rate).astype(float)
        results.append(w)

    return pd.concat(results, ignore_index=True)

recomb_final = load_or_compute(
    tmpfile("tmp_06_recomb_windows_with_rate_quantile.tsv"),
    compute_recomb_rate
)
...
recomb_refined = load_or_compute(
    tmpfile("tmp_07_recomb_windows_refined_quantile.tsv"),
    compute_refined
)
...
def compute_stats_basic():
    data = recomb_final[recomb_final["recombRateRelative"].notna()]
    ...
    df = pd.DataFrame(res)
    df.to_csv(os.path.join(path_out,"stats_by_category_quantile.tsv"),sep="\t",index=False)
    return df
...
def compute_stats_refined():
    data = recomb_refined[recomb_refined["recombRateRelative"].notna()]
...
final_output = os.path.join(path_out,"stats_by_category_refined_quantile.tsv")
