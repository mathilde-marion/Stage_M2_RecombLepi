...
def run_pipeline(genes_df, recomb_df, bin_size, label=""):
    ...
    for chrom in recomb_df["chrom"].unique():
        ...
        # 🔥 CORRECTION MAJEURE : on couvre tout le chromosome
        ...
        rates = [get_rate(p, recomb_chr) for p in centers]
...
for perm in range(args.n_perm):
    ...
    for i, chrom in enumerate(sampled_chroms):
        ...
        start = np.random.randint(
            chrom_min,
            max(chrom_max - length, chrom_min + 1)
        )
...
if all_results:
    ...
    out_path = tmpfile("permutation_null_model.tsv")
