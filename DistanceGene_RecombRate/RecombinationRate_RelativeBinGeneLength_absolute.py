...
#    match = re.search(r"gene_id=([^;]+)", str(attr))
...
for chrom in genes["chrom"].unique():
    ...
    for _, gene in genes_chr.iterrows():
        ...
        # Filtrer les fenêtres recomb qui chevauchent ce gène
        ...
        for i in range(args.bins):
            ...
            for _, rec in rec_overlap.iterrows():
                ...
                if ov > 0:
                    weighted_sum  += ov * rec["recombRate"]
                    total_overlap += ov
...
print(f"  Gènes avec au moins 1 bin renseigné : {df[df['recombRate'].notna()]['gene_id'].nunique()}")
print(f"  NA : {df['recombRate'].isna().sum()} bins ({df['recombRate'].isna().mean()*100:.1f}%)")
